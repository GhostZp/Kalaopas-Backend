...
import {Like, MediaItemWithOwner, TokenContent} from 'hybrid-types/DBTypes';
...
const likeListByUserIdGet = async (
  ...
  res: Response<MediaItemWithOwner[], {user: TokenContent}>,
  next: NextFunction,
) => {
  try {
    ...
    const media = await fetchLikedMediaByUserId(user_id);
    res.json(media);
  } catch (error) {
    next(error);
  }
